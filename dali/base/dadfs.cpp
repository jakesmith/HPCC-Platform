...
class CDistributedFileDirectory: implements IDistributedFileDirectory, public CInterface
{
    ...
    IDistributedFile *getDistributedFile(const char *lname, IUserDescriptor *user, const INode *foreigndali=nullptr, unsigned foreigndalitimeout=FOREIGN_DALI_TIMEOUT, const StringBuffer *metaInfo=nullptr);
...
public:
    ...
    void setMetaInfo(const StringBuffer &_metaInfo)
    {
        metaInfo.clear().append(_metaInfo.length(), _metaInfo.str());
    }
...
IDistributedFile *CDistributedFileDirectory::dolookup(CDfsLogicalFileName &_logicalname, IUserDescriptor *user, bool writeattr, bool hold, bool lockSuperOwner, IDistributedFileTransaction *transaction, unsigned timeout)
{
    ...
    for (;;)
    {
        ...
        if (fileAccessHook) // if configured
        {
             ...
             */
            accessGranted = fileAccessHook->lookup(metaInfo, *logicalname, user, writeattr);
        }
        if (accessGranted)
        {
            if (logicalname->isForeign())
            {
                if (accessGranted)
                {
                    ...
                    return getDistributedFile(logicalname->get(), user, nullptr, FOREIGN_DALI_TIMEOUT, &metaInfo);
                }
            }
            else
            {
                ...
                for (;;)
                {
                    ...
                    if (fcl.getKind() == DXB_File)
                    {
                        ...
                        /* JCSMORE in future, this 'metaInfo' should represent everything we need,
...
IDistributedFile *CDistributedFileDirectory::getDistributedFile(const char *lname, IUserDescriptor *user, const INode *foreigndali, unsigned foreigndalitimeout, const StringBuffer *metaInfo)
{
    Owned<IPropertyTree> tree = getFileTree(lname,user, foreigndali, foreigndalitimeout, false);
    if (!tree)
        return nullptr;
