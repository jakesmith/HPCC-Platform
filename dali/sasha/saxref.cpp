...
public:
    ...
    {
        unsigned oldUsed = usedBytes.fetch_add(sz);
        if ((sz+oldUsed)>maxBytes)
        {
            usedBytes.fetch_sub(sz); // Roll back the increment
            throw makeStringExceptionV(0, "XRefAllocator::alloc : Requested size too large: req: %d, used: %d, max: %d", sz, oldUsed, maxBytes);
        }
