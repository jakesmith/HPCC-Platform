...
void testPattern(const char *type, const char *pattern, unsigned tests, unsigned valueStrSize)
{
    ...
    try
    {
        enum TestType { tt_null, tt_wild, tt_boost, tt_c11, tt_hpcc } testType = tt_null;
        if (strieq("wild", type))
            testType = tt_wild;
        ...
        for (unsigned t=0; t<tests; t++)
        {
            switch (testType)
            {
                ...
                case tt_boost:
                {
                    ...
                    translateToRegExp(pattern, regexPattern);
        ...
        unsigned searchMs =  static_cast<unsigned>(cycle_to_millisec(searchTime));
        unsigned createMs =  static_cast<unsigned>(cycle_to_millisec(createTime));
        PROGLOG("%s regexp, pattern = %s, tests = %u, value size = %u, search time = %u ms, create(exp) time = %u ms", type, pattern, tests, valueStrSize, searchMs, createMs);
    }
    catch (IException *e)
    {
        EXCLOG(e, nullptr);
...
#if 1
    const char *patterns[] = { "abc", "*abc", "abc*", "*abc*", "abc*def", "*abc*def*", nullptr };
