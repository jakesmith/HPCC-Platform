...
void openSSLEncrypt(MemoryBuffer &out, size32_t inLen, const void *inBytes, const char *key, const char *passPhrase)
{
    MemoryBuffer publicKeyMb;
    OwnedIFile iFile = createIFile(key);
    OwnedIFileIO iFileIO = iFile->open(IFOread);
    if (!iFileIO)
        throwStringExceptionV(0, "openSSLEncrypt: failed to open key: %s", key);
    size32_t sz = read(iFileIO, 0, iFile->size(), publicKeyMb);
    iFileIO.clear();

    OwnedEVPObject<BIO, myBIOfree> publicKeyBio(BIO_new_mem_buf(publicKeyMb.bufferBase(), -1));
    RSA *publicKey = PEM_read_bio_RSA_PUBKEY(publicKeyBio, nullptr, nullptr, (void*)passPhrase);
    if (!publicKey)
        throwStringExceptionV(0, "openSSLEncrypt: [PEM_read_bio_RSA_PUBKEY] failed to create key: %s", key);
    OwnedEVPObject<EVP_PKEY, EVP_PKEY_free> encryptKey(EVP_PKEY_new());
    EVP_PKEY_set1_RSA(encryptKey, publicKey);

    OwnedEVPObject<EVP_PKEY_CTX, EVP_PKEY_CTX_free> ctx = EVP_PKEY_CTX_new(encryptKey, nullptr);
    if (!ctx || (EVP_PKEY_encrypt_init(ctx) <= 0) || (EVP_PKEY_CTX_set_rsa_padding(ctx, RSA_PKCS1_PADDING)) <= 0)
        throwStringExceptionV(0, "openSSLEncrypt: failed to initialize key: %s", key);

    /* Determine buffer length */
    size_t outLen;
    if (EVP_PKEY_encrypt(ctx, nullptr, &outLen, (unsigned char *)inBytes, inLen) <= 0)
        throwStringExceptionV(0, "openSSLEncrypt: [EVP_PKEY_encrypt] failed to encrypt with key: %s", key);

    OwnedEVPObject<void, myOpenSSLFree> encrypted = OPENSSL_malloc(outLen);
    assertex(encrypted);

    if (EVP_PKEY_encrypt(ctx, (unsigned char *)encrypted.get(), &outLen, (unsigned char *)inBytes, inLen) <= 0)
        throwStringExceptionV(0, "EVP_PKEY_encrypt");

    out.append(outLen, encrypted);
}

void openSSLDecrypt(MemoryBuffer &out, size32_t inLen, const void *inBytes, const char *key, const char *passPhrase)
{
    MemoryBuffer privateKeyMb;
    OwnedIFile iFile = createIFile(key);
    OwnedIFileIO iFileIO = iFile->open(IFOread);
    if (!iFileIO)
        throwStringExceptionV(0, "openSSLDecrypt: failed to open key: %s", key);
    size32_t sz = read(iFileIO, 0, iFile->size(), privateKeyMb);
    iFileIO.clear();
    ...
    RSA *privateKey = PEM_read_bio_RSAPrivateKey(privateKeyBio, nullptr, nullptr, (void*)passPhrase);
    if (!privateKey)
        throwStringExceptionV(0, "openSSLDecrypt: [PEM_read_bio_RSAPrivateKey] failed to create key: %s", key);
    OwnedEVPObject<EVP_PKEY, EVP_PKEY_free> decryptKey(EVP_PKEY_new());
    EVP_PKEY_set1_RSA(decryptKey, privateKey);

    OwnedEVPObject<EVP_PKEY_CTX, EVP_PKEY_CTX_free> ctx = EVP_PKEY_CTX_new(decryptKey, nullptr);
    if (!ctx || (EVP_PKEY_decrypt_init(ctx) <= 0) || (EVP_PKEY_CTX_set_rsa_padding(ctx, RSA_PKCS1_PADDING)) <= 0)
        throwStringExceptionV(0, "openSSLDecrypt: failed to initialize key: %s", key);

    /* Determine buffer length */
    size_t outLen;
    if (EVP_PKEY_decrypt(ctx, nullptr, &outLen, (const unsigned char *)inBytes, inLen) <= 0)
        throwStringExceptionV(0, "openSSLEncrypt: [EVP_PKEY_decrypt] failed to decrypt with key: %s", key);

    OwnedEVPObject<void, myOpenSSLFree> decrypted = OPENSSL_malloc(outLen);
    assertex(decrypted);

    if (EVP_PKEY_decrypt(ctx, (unsigned char *)decrypted.get(), &outLen, (const unsigned char *)inBytes, inLen) <= 0)
        throwStringExceptionV(0, "openSSLEncrypt: [EVP_PKEY_decrypt] failed to decrypt with key: %s", key);
...
{   
    ...
    try
    {
#if 1
        {
            ...
            const char *publicKey = "/home/jsmith/.ssh/id_rsa.pub.pem";
            const char *privateKey = "/home/jsmith/.ssh/id_rsa";

            StringBuffer message("dhwiuohg oehwvohsohwguhwgfwfwf");

            PROGLOG("Original:\n%s", message.str());

            MemoryBuffer encryptedMessage;
            openSSLEncrypt(encryptedMessage, message.length(), message.str(), publicKey, nullptr);

            StringBuffer encryptedMessageBase64;
            JBASE64_Encode(encryptedMessage.bytes(), encryptedMessage.length(), encryptedMessageBase64);

            PROGLOG("Encrypted:\n%s", encryptedMessageBase64.str());


            /////////////////

            encryptedMessage.clear();
            JBASE64_Decode(encryptedMessageBase64.str(), encryptedMessage);

            MemoryBuffer decrypted;
            openSSLDecrypt(decrypted, encryptedMessage.length(), encryptedMessage.bytes(), privateKey, nullptr);
            StringBuffer decryptedStr(decrypted.length(), (const char *)decrypted.bytes());
