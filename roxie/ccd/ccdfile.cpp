...
public:
    ...
    {
        unsigned activeIdx;
        Owned<IFileIO> active = getCheckOpen(activeIdx); // this object would close on last release
        unsigned tries = 0;
        for (;;)
        {
            try
    ...
    virtual unsigned __int64 getStatistic(StatisticKind kind)
    {
        ...
        CriticalBlock b(crit); // don't bother with linking in crit, because getStatistic is very quick
