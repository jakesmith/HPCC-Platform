...
#ifdef __SAVED_TEMPORARILY_INCASE_NEEDED__
bool RoxieOutputRowArray::add(const void * row, rowidx_t i)
{
    if (numRows == maxRows)
    {
        if (!ensure(numRows+1))
    ...
    else
    {
        ...
        memmove(rows + i + 1, rows + i, (numRows - i) * sizeof(void *));
        rows[i] = row;
        numRows++;
...
bool RoxieOutputRowArray::set(const void * row, rowidx_t i)
{
    if (i >= numRows)
    {
        return add(row, i);
    ...
    else
    {
        ...
        if (i < firstRow)
        {
            ...
            for (unsigned j=i+1; j < firstRow; j++)
                rows[j] = NULL;
            firstRow = i;
        }
        else
