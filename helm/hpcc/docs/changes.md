...
## ServiceAccount Annotations and Labels

The HPCC Helm chart now supports adding annotations to ServiceAccounts and labels to pods that use those ServiceAccounts. This enables integration with cloud provider identity management systems such as Azure Workload Identity, AWS IAM Roles for Service Accounts (IRSA), and Google Cloud Workload Identity.

### New Feature

A new `serviceAccounts` section has been added to the global configuration in values.yaml. This allows you to configure:
- **annotations**: Added to the ServiceAccount resource itself
- **podLabels**: Added to all pods that use the ServiceAccount

### ServiceAccounts Available

The HPCC platform creates the following ServiceAccounts:
- `default`: Used by components that don't need API access (roxie, sasha, etc.)
- `agent`: Used by components that need API access to launch child jobs (eclagent, eclccserver)
- `thoragent`: Used by Thor manager for launching jobs and managing NetworkPolicies
- `esp-service`: Used by ESP components that need to check service status
- `dali`: Used by Dali components
...
For complete documentation including setup instructions for Azure Workload Identity, AWS IRSA, and Google Cloud Workload Identity, see [docs/identity.md](identity.md).

### Backward Compatibility

This feature is fully backward compatible. Existing deployments will continue to work without any changes.
