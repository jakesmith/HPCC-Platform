...
#ifndef _WIN32
std::tuple<std::string, IPropertyTree *, IPropertyTree *> doLoadConfiguration(IPropertyTree *componentDefault, const char * * argv, const char * componentTag, const char * envPrefix, const char * legacyFilename, IPropertyTree * (mapper)(IPropertyTree *), const char *altNameAttribute);
...
class CConfigUpdater : public CInterface
{
    ...
    std::vector<std::tuple<unsigned, std::function<void ()>>> notifyConfigUpdates;
...
public:
    ...
    unsigned addNotifyFunc(std::function<void ()> notifyFunc)
    {
        CriticalBlock b(notifyFuncCS);
        notifyFuncId++;
        notifyConfigUpdates.push_back(std::make_tuple(notifyFuncId, notifyFunc));
        return notifyFuncId;
    }
    void removeNotifyFunc(unsigned funcId)
    {
        CriticalBlock b(notifyFuncCS);
        for (auto iter = notifyConfigUpdates.begin(); iter != notifyConfigUpdates.end(); iter++)
        {
            if (std::get<0>(*iter) == funcId)
            {
                notifyConfigUpdates.erase(iter);
                return;
            }
        }
    }
...
unsigned installConfigUpdateHook(std::function<void ()> notifyFunc)
{
    if (!configFileUpdater)
        throw makeStringException(-1, "installConfigUpdateHook(): configuration updater not installed");
...
{
    if (!configFileUpdater)
        throw makeStringException(-1, "removeConfigUpdateHook(): configuration updater not installed");
    configFileUpdater->removeNotifyFunc(notifyFuncId);
}
#else
unsigned installConfigUpdateHook(std::function<void ()> notifyFunc)
{
    UNIMPLEMENTED_X("installConfigUpdateHook not supported on this platform");
}

void removeConfigUpdateHook(unsigned notifyFuncId)
{
    UNIMPLEMENTED_X("removeConfigUpdateHook not supported on this platform");
}
#endif
...
std::tuple<std::string, IPropertyTree *, IPropertyTree *> doLoadConfiguration(IPropertyTree *componentDefault, const char * * argv, const char * componentTag, const char * envPrefix, const char * legacyFilename, IPropertyTree * (mapper)(IPropertyTree *), const char *altNameAttribute)
{
    Linked<IPropertyTree> newComponentConfig = createPTreeFromIPT(componentDefault);
