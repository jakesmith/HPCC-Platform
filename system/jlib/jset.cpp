...
protected:
    ...
    inline bits_t &getBits(unsigned i) { return bits.element(i); }
...
protected:
    ...
    inline bits_t &getBits(unsigned i) { return mem[i]; }
    inline void setBits(unsigned i, bits_t &m) { } // NOP
    inline void addBitSet(bits_t m)
    {
        if (fixedMemory)
            throw MakeStringException(-1, "CBitSetSingleThreaded with fixed mem cannot expand");
        mb.append(m);
...
class CBitSetSingleThreaded : public CBitSetBase<CBitSetMemoryHelper>
{
    void deserialize(MemoryBuffer &buffer)
    {
        unsigned count;
...
public:
    CBitSetSingleThreaded()
    {
       // In this form, bitSetUnits and mem will be updated when addBitSet expands mb
    }
    CBitSetSingleThreaded(size32_t memSz, const void *_mem, bool reset)
    {
    ...
    CBitSetSingleThreaded(MemoryBuffer &buffer)
    {
        deserialize(buffer);
    }
    virtual void reset()
...
template <class BITSETHELPER>
class CBitSetBase2 : public BITSETHELPER, public CSimpleInterfaceOf<IBitSet>
{
protected:
    typedef BITSETHELPER PARENT;
    using PARENT::getWidth;
...
class CBitSetSingleThreaded2 : public CBitSetBase2<CBitSetMemoryHelper2>
{
    void deserialize(MemoryBuffer &buffer)
    {
        unsigned count;
...
public:
    CBitSetSingleThreaded2()
    {
       // In this form, bitSetUnits and mem will be updated when addBitSet expands mb
    }
    CBitSetSingleThreaded2(size32_t memSz, const void *_mem, bool reset)
    {
        bitSetUnits = memSz*sizeof(byte) / sizeof(bits_t);
        mem = (bits_t *)_mem;
        if (reset)
    ...
    CBitSetSingleThreaded2(MemoryBuffer &buffer)
    {
        deserialize(buffer);
    }
    virtual void reset()
...
extern jlib_decl IBitSet *createBitSetSingleThreaded(unsigned maxBits, const void *mem, bool reset)
{
    return new CBitSetSingleThreaded(maxBits, mem, reset);
}

extern jlib_decl IBitSet *createBitSetSingleThreaded2(unsigned maxBits, const void *mem, bool reset)
{
    return new CBitSetSingleThreaded2(maxBits, mem, reset);
}

extern jlib_decl IBitSet *createBitSetSingleThreaded()
{
    return new CBitSetSingleThreaded();
}
...
{
    return new CBitSetSingleThreaded(mb);
}
