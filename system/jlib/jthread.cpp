            ...
            if (traceStartDelayPeriod)
            {
                ...
                if (timedout)
                {
                    ...
                    if (overAllTimer.elapsedCycles() >= queryOneSecCycles()*traceStartDelayPeriod) // check avg. delay per minute
                    {
                        ...
                        unsigned totalElapsedSecs = static_cast<unsigned>(overAllTimer.elapsedCycles()/60000000);
                        PROGLOG("%s: %d threads started in last %d seconds, total delay = %0.2f milliseconds, average delay = %0.2f milliseconds, currently running = %d", poolname.get(), startsInPeriod, totalElapsedSecs, totalDelayMs, avgDelayMs, runningCount());
                        startsInPeriod = 0;
