...
class jlib_decl ReadWriteLock
{
    ...
    // NB: cs should be locked on entry to this method
    bool doLockWrite(bool timed, unsigned timeout)
    {
    ...
    {
        cs.enter();
        return doLockWrite(timed, timeout);
    }
    bool changeToWrite(bool timed, unsigned timeout)
    {
        ...
        {
            cs.leave();
            return false; // failed because already write locked
        }
        if (readLocks)
            readLocks--;
        return doLockWrite(timed, timeout); // returns false if timed out
    }
public:
    ReadWriteLock()
    {
    ...
    void changeToWrite() { changeToWrite(false, 0); }
    bool changeToWrite(unsigned timeout) { return changeToWrite(true, timeout); }
    void unlock()
    {
        cs.enter(); 
