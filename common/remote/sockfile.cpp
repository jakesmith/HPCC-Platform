...
class CRemoteRequest : public CSimpleInterfaceOf<IInterface>
{
    ...
    void processWrite(IPropertyTree *requestTree, MemoryBuffer &rowDataMb, MemoryBuffer &responseMb)
    {
         ...
         */
        size32_t rowDataSz = rowDataMb.remaining();
        writeActivity->write(rowDataSz, rowDataMb.readDirect(rowDataSz));
    }

public:
    CRemoteRequest(int _cursorHandle, OutputFormat _format, ICompressor *_compressor, IRemoteActivity *_activity)
        : cursorHandle(_cursorHandle), format(_format), compressor(_compressor), activity(_activity)
    {
        if (outFmt_Binary != format)
        {
...
class CRemoteDiskWriteActivity : public CRemoteWriteBaseActivity
{
    ...
    void checkOpen()
    {
        ...
        if (compressionFormat)
            iFileIO.setown(createCompressedFileWriter(iFile, recordSize, false, true, nullptr, compressionFormat));
        else
        {
...
void verifyMetaInfo(IPropertyTree &actNode, bool authorizedOnly, const IPropertyTree *keyPairInfo)
{
    ...
    switch (metaInfoVersion)
    {
        ...
        case 2: // serialized compact IFileDescriptor
        {
...
#if 1
            // JCSMORE - will probably want to keep this for a little longer
            Owned<IFileDescriptor> fileDesc = deserializeFileDescriptorTree(fileInfo, nullptr, IFDSF_EXCLUDE_PARTS);

            // NB: filePart and filePartCopy same as v1
            // extra filename based on part/copy.
            assertex(actNode.hasProp("filePart"));
            unsigned partNum = actNode.getPropInt("filePart");
            assertex(partNum);
            ...
            // assertex(rfn.isLocal()); // this request should never have got here if this part is remote, but this node may have multiple IP's..
...
public:
    ...
    void cmdGetDir(MemoryBuffer &msg, MemoryBuffer &reply, CRemoteClientHandler &client)
    {
        ...
        else
        {
            ...
            reply.append((unsigned)RFEnoerror);
            if (serializeRemoteDirectory(reply,iter,stream?0x100000:0,stream<2))
            {
                if (stream != 0)
                    client.opendir.clear();
            }
    ...
    void cmdMonitorDir(MemoryBuffer &msg, MemoryBuffer &reply, CRemoteClientHandler &client)
    {
        ...
        {
            SocketEndpoint ep;
            IRemoteDirectoryDifferenceIterator *di = createRemoteDirectorIterator(ep, name);
            di->appendBuf(msg);
            prev.setown(di);
        }
        if (TF_TRACE)
    ...
    void cmdStreamCommon(IPropertyTree *requestTree, MemoryBuffer &rest, MemoryBuffer &reply, CRemoteClientHandler &client)
    {
        ...
        if (!cursorHandle)
        {
            ...
            {
                const char *commCompressionType = requestTree->queryProp("outputCompression");
                if (isEmptyString(commCompressionType))
                    commCompressionType = requestTree->queryProp("commCompression");
            ...
            remoteRequest.setown(new CRemoteRequest(cursorHandle, outputFormat, compressor, outputActivity));
