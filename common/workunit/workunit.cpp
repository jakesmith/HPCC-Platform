...
void executeThorGraph(const char * graphName, IConstWorkUnit &workunit, const IPropertyTree &config)
{
    ...
    if (config.hasProp("@queue"))
    {
        ...
        // Look for an existing Thor instance that has been used for this job.
     ...
     * (in bare-metal it is returned via a socket the agent connects to Thor with)
     */
...
bool executeGraphOnLingeringThor(IConstWorkUnit &workunit, const char *graphName)
{
     ...
     *
     * Lingering Thor's register when they are idle with the workunit itselfn
     ...
     * instance.
     * Or, in the case of a multi-linger Thor configuration, may be picked up
     * by one of the multi-linger thor's that is listening to the thor queue for the 
     * linger period.
     */
