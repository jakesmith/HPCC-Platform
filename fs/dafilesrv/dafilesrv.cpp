...
int main(int argc, const char* argv[])
{
    ...
    StringBuffer certificateName;
    if (!getComponentConfigSP()->getProp("@certificate", certificateName))
        certificateName.set("local");
    IPropertyTree *info = queryMtlsSecretInfo(certificateName);
    if (!info)
    {
        ERRLOG("No '%s' MTLS certificate detected.", certificateName.str());
        return 0; // signal 'success' to avoid pod restart
    }
    keyPairInfo.setown(createPTree());
    IPropertyTree *keyPair = keyPairInfo->setPropTree("KeyPair");
    keyPair->setProp("@name", certificateName);

    const char *verifyCertificatePath = info->queryProp("verify/ca_certificates/@path");
    if (isEmptyString(verifyCertificatePath))
    {
        ERRLOG("local MTLS verify certificate path missing.");
        return 0; // signal 'success' to avoid pod restart
    }
    keyPair->setProp("@publicKey", verifyCertificatePath);

    connectMethod = SSLOnly;
    dedicatedRowServicePort = 0; // row service always runs on same secure ssl port in containerized mode
    port = 0;
    sslport = getComponentConfigSP()->getPropInt("@port", SECURE_DAFILESRV_PORT);
