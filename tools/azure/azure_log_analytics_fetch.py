...
try:
    ...
    print("Error: Azure SDK packages not installed.", file=sys.stderr)
    print("Please install required packages: pip install azure-identity azure-monitor-query", file=sys.stderr)
    sys.exit(1)
...
Examples:
  ...
  %(prog)s --subscription-id <sub-id> --workspace-id <workspace-id> \\
           --tenant-id <tenant> --client-id <client> --client-secret <secret> \\
           --aks-name <aks> --start "2024-01-01T00:00:00Z" \\
           --end "2024-01-01T23:59:59Z" --output results.csv
        """
    )
    ...
    auth_group.add_argument('--tenant-id', help='Azure tenant ID (optional, for service principal auth)')
    auth_group.add_argument('--client-id', help='Azure client ID (optional, for service principal auth)')
    auth_group.add_argument('--client-secret', help='Azure client secret (optional, for service principal auth)')
...
def create_credential(args):
    """Create Azure credential based on provided arguments."""
...
def query_log_analytics(credential, workspace_id: str, query: str, verbose: bool = False) -> List[Dict]:
    """
    ...
    if verbose:
        print(f"Executing query against workspace {workspace_id}")
        print(f"Query:\n{query}\n")
    ...
    try:
        response = client.query_workspace(
            ...
            query=query,
            timespan=None  # timespan is specified in the query
        )
        
        if response.status == LogsQueryStatus.PARTIAL:
            print("Warning: Partial results returned", file=sys.stderr)
...
def main():
    ...
    try:
        ...
        # Execute query
        results = query_log_analytics(credential, args.workspace_id, query, args.verbose)
