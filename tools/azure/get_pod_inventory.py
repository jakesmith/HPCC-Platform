#!/usr/bin/env python3
"""
Get Azure Kubernetes Pod Inventory from Log Analytics

This script queries the KubePodInventory table in Azure Log Analytics to retrieve
information about pods running in a specific namespace at a given time.

Usage:
    ./get_pod_inventory.py --start-time "2025-11-04 12:00" [options]

Examples:
    # Auto-discover from kubectl context
    ./get_pod_inventory.py --start-time "2025-11-04 12:00"

    # Specify resource group to search for AKS cluster
    ./get_pod_inventory.py --start-time "2025-11-04 12:00" -g myresourcegroup

    # Query specific namespace with time range
    ./get_pod_inventory.py --start-time "2025-11-04 09:00" \
      --end-time "2025-11-04 17:00" -n hpcc

    # Filter and export
    ./get_pod_inventory.py --start-time "2025-11-04 12:00" \
      --status Running --limit 50 --format json --export pods.json

DateTime format: YYYY-MM-DD HH:MM (defaults to EST, use --utc for UTC)
"""
...
def query_log_analytics(workspace_id, resource_group, subscription, query, verbose=False):
...
def build_query(namespace, start_time, end_time, status_filter=None, limit=None):
    """Build the KQL query for pod inventory."""
...
"""
    ...
    if status_filter:
        query += f"| where PodStatus == '{status_filter}'\n"

    query += """| summarize arg_max(TimeGenerated, *) by Name
| project Name, PodStatus, Computer, ContainerStatus, PodCreationTimeStamp
| sort by Name asc"""
...
def format_output(results, output_format='table', metadata=None):
    ...
    if output_format == 'json':
        ...
        if metadata:
            lines.append("# Generated by: get_pod_inventory.py")
            ...
            if metadata.get('status'):
                lines.append(f"# Status filter: {metadata.get('status', '')}")
            lines.append("#")
        ...
        headers = ['Name', 'Status', 'Node', 'Container', 'Created']
        col_widths = {h: len(h) for h in headers}

        for row in results:
            col_widths['Name'] = max(col_widths['Name'], len(row.get('Name', '')))
            col_widths['Status'] = max(col_widths['Status'], len(row.get('PodStatus', '')))
            col_widths['Node'] = max(col_widths['Node'], len(row.get('Computer', '')))
            col_widths['Container'] = max(col_widths['Container'], len(row.get('ContainerStatus', '')))
            created = row.get('PodCreationTimeStamp', '')
            col_widths['Created'] = max(col_widths['Created'], len(str(created)))

        # Limit column widths for readability
        col_widths['Name'] = min(col_widths['Name'], 60)
        col_widths['Node'] = min(col_widths['Node'], 40)
        ...
        for row in results:
            values = [
                row.get('Name', '')[:60].ljust(col_widths['Name']),
                row.get('PodStatus', '').ljust(col_widths['Status']),
                row.get('Computer', '')[:40].ljust(col_widths['Node']),
                row.get('ContainerStatus', '').ljust(col_widths['Container']),
                str(row.get('PodCreationTimeStamp', ''))[:col_widths['Created']].ljust(col_widths['Created'])
            ]
            lines.append(' | '.join(values))
...
def main():
    parser = argparse.ArgumentParser(
        description='Query Kubernetes pod inventory from Azure Log Analytics',
...
Examples:
  ...
  %(prog)s --workspace-id <workspace-id> -n hpcc --start-time "2025-11-04 17:00" --utc
  %(prog)s --workspace-id <workspace-id> -n hpcc --start-time "2025-11-04 12:00" --status Running --limit 50
  %(prog)s --workspace-id <workspace-id> -n hpcc --start-time "2025-11-04 12:00" --format json > pods.json
        """
    )
    ...
    parser.add_argument('--utc', action='store_true', help='Treat times as UTC (default is EST/EDT)')
    ...
    parser.add_argument('--status', choices=['Running', 'Pending', 'Succeeded', 'Failed', 'Terminating'],
                        help='Filter by pod status')
    ...
    # Convert to UTC if needed (EST is UTC-5, EDT is UTC-4)
    if not args.utc:
        # Assume EST (UTC-5) for simplicity
        # For production, you'd want to handle DST properly
    ...
    if args.end_time:
        ...
        # Convert end time to UTC if needed
        if not args.utc:
    ...
    if namespace:
        print(f"Querying pod inventory for namespace '{namespace}'", file=sys.stderr)
    else:
        print(f"Querying pod inventory for ALL namespaces", file=sys.stderr)
    ...
    query = build_query(namespace, start_time, end_time, args.status, args.limit)
    ...
    results = query_log_analytics(workspace_id, resource_group, subscription_id, query, args.verbose)
    ...
    # Format output
    output = format_output(results, args.format, metadata=metadata)

    # Display summary
    print(f"Found {len(results)} pods", file=sys.stderr)
    if args.status:
