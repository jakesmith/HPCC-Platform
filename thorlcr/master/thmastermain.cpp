...
// These need to be re-merged when the configuration is refreshed.
...
public:
    ...
    void connect(unsigned workers)
    {
        ...
        // These settings will be re-merged when config is refreshed
        {
            ...
            // Copy properties that the manager added to globals and need to be merged on workers
            if (globals->hasProp("@masterBuildTag"))
            ...
            if (globals->hasProp("@name"))
                managerAdditionalSettings->setProp("@name", globals->queryProp("@name"));
            if (globals->hasProp("@nodeGroup"))
                managerAdditionalSettings->setProp("@nodeGroup", globals->queryProp("@nodeGroup"));
            if (globals->hasProp("@masterTotalMem"))
                managerAdditionalSettings->setPropInt("@masterTotalMem", globals->getPropInt("@masterTotalMem"));
            if (globals->hasProp("@thorPath"))
            ...
            IPropertyTree *managerMemory = globals->queryPropTree("managerMemory");
            if (managerMemory)
                managerAdditionalSettings->setPropTree("managerMemory", createPTreeFromIPT(managerMemory));
        }
#endif
...
int main( int argc, const char *argv[]  )
{
    ...
    {
#ifdef _CONTAINERIZED
        // In containerized mode, enable config monitoring
        // Additional manager settings will be sent separately to workers and re-merged on config refresh
        bool monitorConfig = true;
#else
        // Do not allow updates to the config file in bare-metal, otherwise the slave may not be in sync.
