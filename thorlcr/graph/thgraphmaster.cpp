        ...
        loop
        {
            ...
            switch (msgType)
            {
                ...
                case smt_getresult:
                {
                    ...
                    unsigned resultId;
                    msg.read(resultId);
                    CJobChannel &jobChannel = job.queryJobChannel(slave);
...
// ICodeContext
    ...
    virtual unsigned getNodes() { return jobChannel.queryJob().queryJobGroup().ordinality()-1; }
...
{
    jobChannels.append(*new CJobMasterChannel(*this, mpServer, 0));
}
    ...
    if (!aborted)
    {
        ...
        else
        {
            if (isGlobal())
            {
                ...
                try
                {
                    jobM->broadcast(queryNodeComm(), msg, mpTag, LONGTIMEOUT, "serializeCreateContexts", &bcastMsgHandler);
                }
                catch (IException *e)
                {
...
bool CMasterGraph::preStart(size32_t parentExtractSz, const byte *parentExtract)
{
    ...
    if (!sentStartCtx || reinit)
    {
        ...
        try
        {
            jobM->broadcast(queryClusterComm(), msg, mpTag, LONGTIMEOUT, "startCtx", NULL, true);
        }
        catch (IException *e)
        {
...
void CMasterGraph::getFinalProgress()
{
    ...
    while (n--)
    {
        ...
        unsigned slavesPerProcess = globals->getPropInt("@slavesPerProcess", 1); // JCSMORE - should move somewhere common
