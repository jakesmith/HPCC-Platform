...
public:
    CThorSlaveGraphResults(CJobSlave &_job,unsigned numResults) : CThorGraphResults(numResults), job(_job)
    {
    }
    ...
    IThorResult *getResult(unsigned id, bool distributed)
    {
        ...
        Owned<IThorResult> gr = job.getGlobalResult(*result->queryActivity(), result->queryRowInterfaces(), ownerId, id);
...
IThorGraphResults *CJobSlave::createThorGraphResults(unsigned num)
{
    return new CThorSlaveGraphResults(*this, num);
}

IThorResult *CJobSlave::getGlobalResult(ILWActivity &activity, IRowInterfaces *rowIf, activity_id ownerId, unsigned id)
{
    mptag_t replyTag = createReplyTag();
    CMessageBuffer msg;
    msg.setReplyTag(replyTag);
    msg.append(smt_getresult);
    msg.append(activity.queryGraphId());
    msg.append(ownerId);
    msg.append(id);
    msg.append(replyTag);
