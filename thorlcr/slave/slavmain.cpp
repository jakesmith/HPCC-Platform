...
class CKJService : public CSimpleInterfaceOf<IKJService>, implements IThreaded, implements IExceptionHandler
{
    ...
    public:
        ...
        const char *queryTracing() const { return fname; }
    };
    struct FetchKey
    {
        StringAttr tracing;
        FetchKey(MemoryBuffer &mb, unsigned _slave) : slave(_slave)
        {
            mb.read(id);
            mb.read(partNo);
            VStringBuffer tracingStr("actId=%u, partNo=%u, slave=%u", id, partNo, slave);
            tracing.set(tracingStr);
        }
        activity_id id;
        unsigned partNo;
        unsigned slave;
        ...
        const char *queryTracing() const { return tracing; }
    };
    ...
    public:
        ...
        void setTranslation(RecordTranslationMode _translationMode, IOutputMetaData *_expectedFormat, unsigned _expectedFormatCrc, IOutputMetaData *_publishedFormat, unsigned _publishedFormatCrc, IOutputMetaData *_projectedFormat)
        {
            translationMode = _translationMode;
            expectedFormat.set(_expectedFormat);
            expectedFormatCrc = _expectedFormatCrc;
    ...
    public:
            ...
            : service(_service), ctx(_ctx)
        {
            ...
            const IDynamicTransform *translator = ctx->queryTranslator(ctx->queryKey().queryTracing());
            if (translator)
    ...
    class CFetchLookupRequest : public CLookupRequest
    {
        ...
        void processRow(const void *row, CFetchLookupResult &reply)
        {
            ...
            Owned<IFileIO> iFileIO = activityCtx->getFetchFileIO(fetchContext->queryKey().partNo);
            ...
            if (translator)
            {
                ...
                fetchedLen = translator->translate(fetchDiskRowBuilder, row);
                prefetchBuffer.finishedRow();
            }
            else
            {
                CThorStreamDeserializerSource ds(stream);
                fetchedLen = activityCtx->queryFetchDiskDeserializer()->deserialize(fetchDiskRowBuilder, ds);
            }
    ...
    public:
        CFetchLookupRequest(CFetchContext *_fetchContext, rank_t _sender, mptag_t _replyTag)
            : CLookupRequest(_fetchContext->queryActivityCtx(), _sender, _replyTag), fetchContext(_fetchContext)
        {
            allocator = activityCtx->queryFetchInputAllocator();
            deserializer = activityCtx->queryFetchInputDeserializer();
            translator = fetchContext->queryTranslator(fetchContext->queryKey().queryTracing());
            if (translator)
    ...
    CKeyLookupContext *ensureKeyLookupContext(CJobBase &job, const CLookupKey &key)
    {
        CriticalBlock b(lCCrit);
            ...
            try
            {
                ...
                switch (cmd)
                {
                    case kjs_open:
                    {
                        ...
                        Owned<CKeyLookupContext> keyLookupContext = ensureKeyLookupContext(*currentJob, key); // ensure entry in keyLookupContextsHT, will be removed by last CKMContainer
                        ...
                        if (RecordTranslationMode::None != translationMode)
                        {
                            ...
                            IHThorKeyedJoinArg *helper = keyLookupContext->queryHelper();
                            ...
                            keyLookupContext->setTranslation(translationMode, expectedFormat, expectedFormatCrc, publishedFormat, publishedFormatCrc, projectedFormat);
                        }
                    ...
                    case kjs_fetchopen:
                    {
                        FetchKey key(msg, (unsigned)sender);
                        ...
                        if (RecordTranslationMode::None != translationMode)
                        {
                            ...
                            dbgassertex(expectedFormatCrc); // translation mode wouldn't have been set unless available
                            fetchContext->setTranslation(translationMode, expectedFormat, expectedFormatCrc, publishedFormat, publishedFormatCrc, projectedFormat);
                        }
...
// IKJService
    virtual void setCurrentJob(CJobBase &job)
    {
        ...
        keyLookupMaxProcessThreads = job.getOptUInt("keyedJoinMaxProcessors", defaultKeyLookupMaxProcessThreads);
    }
