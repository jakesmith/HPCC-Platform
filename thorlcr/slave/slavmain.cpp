            ...
            try
            {
                ...
                switch (cmd)
                {
                    case QueryInit:
                    {
                        ...
                        msg.swapWith(mb);
                        mptag_t slaveMsgTag, kJServiceTag;
                        deserializeMPtag(msg, slaveMsgTag);
                        queryNodeComm().flush(slaveMsgTag);
                        deserializeMPtag(msg, kJServiceTag);
                        queryNodeComm().flush(kJServiceTag);
                    ...
                    // only relevant if dllsToSlaves=false and query_so_dir was fully qualified remote path (e.g. //<ip>/path/file
                        ...
                        Owned<CJobSlave> job = new CJobSlave(watchdog, workUnitInfo, graphName, querySo, slaveMsgTag, kJServiceTag);
                        job->setXGMML(deps);
