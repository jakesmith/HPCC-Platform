...
class CKJService : public CSimpleInterfaceOf<IKJService>, implements IThreaded, implements IExceptionHandler
{
    ...
    public:
        ...
        virtual void beforeDispose() override
        {
            PROGLOG("CKeyLookupContext::beforeDispose(): key=%s", key.fname.get());
        }
        ~CKeyLookupContext()
        {
            PROGLOG("~CKeyLookupContext(): key=%s", key.fname.get());
        }
    ...
    public:
            ...
            : service(_service), ctx(_ctx)
        {
            ...
            handle = service.getUniqId();
            PROGLOG("CKMContainer() key=%s", ctx->queryKey().fname.get());
        }
        virtual void beforeDispose() override
        {
            PROGLOG("CKMContainer::beforeDispose() key=%s", ctx->queryKey().fname.get());
        }
        ~CKMContainer()
        {
            PROGLOG("~CKMContainer() key=%s", ctx->queryKey().fname.get());
...
public:
    ...
    CKJService(mptag_t _mpTag) : threaded("CKJService", this), keyLookupMpTag(_mpTag)
    {
        ...
        processorPool.setown(createThreadPool("KJService processor pool", factory, this, keyLookupMaxProcessThreads, INFINITE));
    }
    ~CKJService()
    {
        stop();
