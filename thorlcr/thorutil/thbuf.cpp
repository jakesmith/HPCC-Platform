    ...
    public:
        ...
        const void *nextRow()
        {
            ...
            if (row == rowsInRowSet)
            {
                ...
                {
                    totalRows = parent.readerWait(*this, rowsRead);
                    if (0 == totalRows)
                    {
                        doStop();
                        return NULL;
            ...
            if (lastWasNull)
            {
                ...
                {
                    ActPrintLog(parent.activity, "rowsRead = %" RCPF "u, totalRows = %" RCPF "u, writeAtEof = %s, rowsWritten = %" RCPF "u, readersWaiting = %u", rowsRead, totalRows, boolToStr(parent.writeAtEof), parent.rowsWritten, parent.readersWaiting);
                    ActPrintLog(parent.activity, "Double null row detected in COutput::nextRow");
                    hold("Double null row detected in COutput::nextRow");
                }
            }
            else
            {
...
// ISharedSmartBufferRowWriter
    ...
    virtual void putRow(const void *row, ISharedSmartBufferCallback *callback) override
    {
        ...
        {
            ActPrintLog(activity, "Double put null at row %" RCPF "d", rowsWritten);
            hold("Double put null");
        }
        else
            lastPutNull = true;
...
class CSharedWriteAheadMem2 : public CInterfaceOf<ISharedRowStreamReader>
{
    CActivityBase *activity;
    unsigned outputCount = NotFound;
    rowidx_t maxRows;
    Linked<IOutputMetaData> meta;
    Linked<IOutputRowSerializer> serializer;
    Semaphore fullSem;
    ...
    bool stopped = false;
    bool writeAtEof = false;
    IArrayOf<IRowStream> outputs;
    CThorSpillableRowArray rows;
    std::vector<size32_t> rowSizes;
    rowcount_t rowsWritten = 0;
    memsize_t maxMemUsage = 0;
    memsize_t totalMemSz = 0;
    CriticalSection cs;

    class COutput : public CSimpleInterfaceOf<IRowStream>
    {
        CSharedWriteAheadMem2 &parent;
        unsigned output = NotFound;
        rowcount_t rowsRead = 0;
        bool eof, readerWaiting;
        Semaphore readWaitSem;
        rowcount_t row = 0;
        rowcount_t currentHighRowNum = 0;
    ...
    public:
        ...
        const void *nextRow()
        {
            if (eof)
                return nullptr;
            if (row == currentHighRowNum)
            {
                currentHighRowNum = parent.readRows();
                if (row == currentHighRowNum)
                {
                    totalRows = parent.readerWait(*this, rowsRead);
                    if (0 == totalRows)
    ...
    friend class CSharedWriteAheadBase2;
    };
    ...
    virtual rowcount_t readRows(unsigned output)
    {
        rows.readBlock();
...
public:
    CSharedWriteAheadMem2(CActivityBase *_activity, unsigned _outputCount, IThorRowInterfaces *rowif, unsigned _maxMemUsage)
        : activity(_activity), rows(_activity, rowif), , meta(rowIf->queryRowMetaData()), serializer(rowIf->queryRowSerializer()),
          maxMemUsage(_maxMemUsage)
    {
        outputCount = _outputCount;
        for (unsigned c=0; c<outputCount; c++)
            outputs.append(* new COutput(*this, c));
    }
    ~CSharedWriteAheadMem2()
    {
    }
// ISharedSmartBuffer
    virtual ISharedSmartBufferRowWriter *getWriter() override
...
// ISharedSmartBufferRowWriter
    virtual void putRow(const void *row, ISharedSmartBufferCallback *callback) override
    {
        ...
        size32_t len=rowSize(row);
        rows.append(row);
        rowSizes.push_back(len);
        totalMemSz += len;
        rowsWritten++; // including NULLs(eogs)
        if (totalMemSz >= maxMemUsage)
        {
            CLeavableCriticalBlock b(cs);
            if (totalMemSz >= maxMemUsage)
            {
                writeBlocked = true;
                b.leave();
...
ISharedSmartBuffer *createSharedSmartMemBuffer2(CActivityBase *activity, unsigned outputs, IThorRowInterfaces *rowIf, unsigned buffSize)
{
    return new CSharedWriteAheadMem2(activity, outputs, rowIf, buffSize);
}
