    ...
    public:
        ...
        const void *nextRow()
        {
            ...
            if (lastWasNull)
            {
                ...
                {
                    ActPrintLog(parent.activity, "Double null - rowsRead = %" RCPF "u, totalRows = %" RCPF "u, writeAtEof = %s, rowsWritten = %" RCPF "u, readersWaiting = %u", rowsRead, totalRows, boolToStr(parent.writeAtEof), parent.rowsWritten, parent.readersWaiting);
                    kill(getpid(), SIGSTOP); // Send SIGSTOP to the process to freeze all threads
