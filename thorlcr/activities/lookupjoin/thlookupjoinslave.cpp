...
class CMarker
{
    ...
    rowidx_t unique;
    rowidx_t minForParallel, parallelMinChunkSize, parallelChunkSize;
    ...
    rowidx_t getMore(rowidx_t &_startRow) // NB: returns end row #
    {
        ...
        if (endRow-startRow <= chunkSize)
            ...
            startRow += chunkSize;
        return startRow; // now end row (and next startRow)
    }
...
public:
    ...
    rowidx_t calculate(CThorExpandingRowArray &rows, ICompare *_cmp)
    {
        cmp = _cmp;
        ...
        if ((1 == threadCount) || (rowCount < minForParallel))
        {
        ...
        if (needGlobal)
        {
            ...
            rowidx_t uniqueKeys = 0;
            {
                ...
                if (!hasBroadcastSpilt())
                {
                    ...
                    uniqueKeys = marker.calculate(rhs, compareRight);
                    rhsCollated = true;
                }
            }
