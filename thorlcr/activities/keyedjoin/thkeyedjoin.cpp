...
class CKeyedJoinMaster : public CMasterActivity
{
    ...
    void mapParts(CMap &map, IDistributedFile *file, bool isIndexWithTlk, bool allLocal)
    {
            ...
            if (!kind || !strsame("topLevelKey", kind))
            {
                ...
                if (local)
                {
                    if (container.queryLocalData())
                    {
                        ...
                        {
                            std::vector<unsigned> &slaveParts = map.querySlaveParts(subPartIdx);
                            ...
                            if ((partNode->equals(&groupNode)))
                            {
                                if (!partsOnSlaves->testSet(groupSize*p+gn))
                                {
                                    ...
                                    if (NotFound == mappedPos)
                                    {
                                        ...
                                        if (queryJob().queryChannelsPerSlave()>1)
        ...
        if (indexFile)
        {
            ...
            if (helper->diskAccessRequired())
            {
                ...
                if (fetchFilename)
                {
                    ...
                    if (dataFile)
                    {
                        ...
                        mapParts(dataMap, dataFile, false, true);
                    }
                }
            }
            if (!helper->diskAccessRequired() || dataFileDesc)
            {
                ...
                if (totalIndexParts)
                {
                    ...
                    mapParts(indexMap, indexFile, keyHasTlk, getOptBool("allLocalIndexParts"));
