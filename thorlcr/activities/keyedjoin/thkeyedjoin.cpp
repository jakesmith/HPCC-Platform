...
class CKeyedJoinMaster : public CMasterActivity
{
    ...
    enum MapLocal { map_localnop, map_localonly, map_localfirst, map_localall };
    void mapParts(CMap &map, IDistributedFile *file, bool isIndexWithTlk, MapLocal local)
    {
        ...
        for (unsigned p=0; p<numParts; p++)
        {
            ...
            {
                unsigned copies = (map_localonly==local) ? 1 : part->numCopies();
...
//                                    mappedPos = gn;
                                    mappedPos = gn % queryJob().queryJobChannels();
                                    nextGroupStartPos = gn+1;
                                    if (nextGroupStartPos == groupSize)
                                        nextGroupStartPos = 0;
                                }
                                else if (map_localall == local) // all slaves get all locally accessible parts
                                    slaveParts.push_back(partCopy);
                            }
                        }
                        gn++;
                    ...
                    if (NotFound == mappedPos)
                    {
                        if (map_localonly == local)
                            map.slavePartMap[p % groupSize].push_back(p);
                        else
                        {
        ...
        if (indexFile)
        {
            ...
            if (helper->diskAccessRequired())
            {
                ...
                if (fetchFilename)
                {
                    ...
                    if (dataFile)
                    {
                        ...
                        mapParts(dataMap, dataFile, false, container.queryLocalData() ? map_localonly : map_localall);
                    }
                }
            }
            if (!helper->diskAccessRequired() || dataFileDesc)
            {
                ...
                if (totalIndexParts)
                {
                    ...
                    MapLocal local = (container.queryLocalData() || !getOptBool("allLocalIndexParts")) ? map_localonly : map_localall;
                    mapParts(indexMap, indexFile, keyHasTlk, local);
